/* =========================================================================
 * api/master.js — Master Agent router (restored + Phase A thinking)
...
function buildSystemPrompt(mcpTools) {
  var base =
    ...
    '{"agent_id":"image|music|video|image2html|html2image|tts|code|web|chat|mcp|browse",' +
    ...
    '- browse → endpoint kernel.browse, params {url or prompt} for live page visit via Kernel browser\n' +
...
function buildThinking(message, route, mcpTools) {
  var agent = (route && route.agent_id) || 'chat';
  var lines = [];
  lines.push('1. Understood the request: "' + String(message || '').replace(/\s+/g, ' ').trim().slice(0, 120) + '"');
  if (agent === 'mcp') {
    var tool = (route.params && (route.params.tool || route.params.name)) || 'tool';
    var sid = (route.params && route.params.serverId) || '';
    lines.push('2. This looks like live external data — prefer a connected MCP tool.');
    lines.push('3. Selected MCP tool: ' + tool + (sid ? ' (server ' + sid + ')' : '') + '.');
    lines.push('4. Next: run the tool, then format the result in plain language (not raw JSON).');
  } else if (agent === 'image' || agent === 'video' || agent === 'music') {
    lines.push('2. Classified as a generative media request (' + agent + ').');
    lines.push('3. Will route to the ' + agent + ' pipeline; you confirm with Execute (uses quota).');
    lines.push('4. After generation, the result is shown with a short caption.');
  } else if (agent === 'tts') {
    lines.push('2. Classified as text-to-speech.');
    lines.push('3. Will synthesize audio from the provided text when you Execute.');
  } else if (agent === 'browse') {
    lines.push('2. Needs a live browser session (Kernel).');
    lines.push('3. Will open the page, extract readable text, then close the session.');
    lines.push('4. If Kernel fails, fall back to web/chat models so the user still gets an answer.');
  } else if (agent === 'code') {
    lines.push('2. Classified as a code compile/convert task.');
    lines.push('3. Will use the code tool; review output before relying on it.');
  } else if (agent === 'web') {
    lines.push('2. Needs up-to-date or external context.');
    lines.push('3. Will answer with web-capable chat models when available.');
  } else {
    lines.push('2. No specialized media/MCP tool required — answer with chat.');
    if (mcpTools && mcpTools.length) {
      lines.push('3. Note: ' + mcpTools.length + ' MCP tool(s) are connected if you need live data later.');
    } else {
      lines.push('3. No MCP servers connected; using built-in tools only.');
    }
    lines.push('4. Format the reply clearly for a human reader.');
  }
  return lines.join('\n');
}

function attachThinking(route, message, mcpTools) {
  if (!route || typeof route !== 'object') return route;
  var thinking = buildThinking(message, route, mcpTools);
  route.thinking = thinking;
  if (!route.reasoning || String(route.reasoning).indexOf('Heuristic:') === 0) {
    route.reasoning = (route.reasoning ? String(route.reasoning) + ' · ' : '') + 'Plan ready (see thinking).';
  }
  return route;
}
...
function heuristicRoute(message, mcpTools) {
  ...
  if (/\b(browse|visit (this |the )?page|open (this |the )?url|scrape|fetch (the )?page)\b/.test(m) || (/https?:\/\//.test(message) && /\b(browse|visit|open|scrape|read)\b/.test(m)))
    return { agent_id: 'browse', endpoint: 'kernel.browse', params: { prompt: message }, reasoning: 'Heuristic: Kernel browse' };
  if (/\b(search the web|look up|latest news|current events|what is the (current|latest))\b/.test(m))
  ...
  if (mcpTools && mcpTools.length) {
    ...
    for (var i = 0; i < mcpTools.length; i++) {
      ...
      if (t.qualified && m.indexOf(String(t.qualified).toLowerCase()) !== -1) {
        return { agent_id: 'mcp', endpoint: 'mcp.call', params: { serverId: t.serverId, tool: t.name }, reasoning: 'Heuristic: explicit MCP tool ' + t.qualified };
      }
    }
    if (wantsMcp) {
      var pick = mcpTools.find(function (x) { return x.name === 'execute'; }) || mcpTools[0];
      return { agent_id: 'mcp', endpoint: 'mcp.call', params: { serverId: pick.serverId, tool: pick.name }, reasoning: 'Heuristic: MCP for crypto/data query via ' + (pick.serverName || pick.qualified) };
    }
  }
  return null;
}
...
async function callChat(provider, modelCfg, messages, maxTokens) {
  ...
  var body = { model: modelCfg.model, messages: messages, max_tokens: maxTokens || 400 };
...
function sanitizeParams(params) {
  var allowed = {
    ...
    serverId: 1, tool: 1, name: 1, qualified: 1, url: 1
  };
  var out = {};
...
function buildCapabilitiesText(mcpTools) {
  var text =
    ...
    '| **Browse (Kernel)** | Browse https://example.com | uses Kernel credits |\n' +
...
module.exports = async function handler(req, res) {
  try {
    ...
    route = attachThinking(route, message, mcpTools);
    ...
    if (owner !== undefined && owner !== route.agent_id && !validForShared && route.agent_id !== 'mcp' && route.agent_id !== 'browse') {
    ...
    if (route.agent_id === 'browse' || route.endpoint === 'kernel.browse') {
      var browseUrl = (routedParams && routedParams.url) || null;
      var kOut = kernelLib
        ? await kernelLib.tryBrowsePage({ url: browseUrl, prompt: message })
        : { ok: false, error: 'kernel-lib not loaded', code: 'no_lib' };

      if (kOut && kOut.ok) {
        res.status(200).json({
          agent_id: 'browse',
          endpoint: 'kernel.browse',
          params: routedParams,
          result: kOut.result,
          source: 'kernel',
          server_executed: true,
          reasoning: typeof route.reasoning === 'string' ? route.reasoning : 'Kernel browse',
          thinking: route.thinking || null,
          fallback_note: fallbackNote,
          model_used: modelUsed,
          fallback_used: false,
          session_id: kOut.session_id || null
        });
        return;
      }

      var kErr = (kOut && kOut.error) ? kOut.error : 'Kernel unavailable';
      var genFb = await tryGenerateAnswer(
        message + '\n\n(Note: live browser failed (' + kErr.slice(0, 120) + '); answer from general knowledge / web-capable models.)',
        history,
        prefs,
        mcpTools
      );
      res.status(200).json({
        ...
        params: { prompt: message },
        result: genFb.text || ('Live browser unavailable. ' + kErr),
        source: genFb.text ? (genFb.provider || 'llm') : 'error',
        server_executed: true,
        generation_attempts: genFb.attempts,
        reasoning: (route.reasoning || 'Kernel browse') + ' → fallback web',
        thinking: route.thinking || null,
        fallback_note: (fallbackNote ? fallbackNote + ' ' : '') + 'Kernel failed (' + kErr.slice(0, 160) + '); used chat/web fallback.',
        model_used: genFb.model || modelUsed,
        fallback_used: true
      });
      return;
    }
        ...
        var messages = [
          {
            ...
            'You CAN route users to real tools: image, video, music, TTS, code generation, MCP, and Kernel browse. ' +
