...
(function (global) {
  ...
  function loadPrefs() {
    try {
      ...
      if (u && global.Settings) return Settings.load(u.email) || {};
    } catch (_) {}
    return {};
  }
      ...
      try {
        ...
        var res = await fetch('/api/image', {
          ...
          body: JSON.stringify({
            ...
            size: (params && params.size) || prefs.imageSize || undefined,
            provider: (params && params.provider) || prefs.imageProvider || 'auto'
          })
        });
      ...
      try {
        ...
        var res = await fetch('/api/video', {
          ...
          body: JSON.stringify({
            ...
            provider: (params && params.provider) || prefs.videoProvider || 'auto'
          })
        });
        var data = await res.json().catch(function () { return {}; });
        if (!res.ok) {
