 ...
 *
 * v3 changes:
 *   • Hub.refresh() re-renders quota + cards (called on login / settings
 *     save / quota change).
 *   • No more reset button wiring — quota refreshes itself at midnight.
 *   • Compact-cards setting is honored.
 * ========================================================================= */
...
(function () {
  ...
  const TOOLS = [
    ...
    { id: 'tts',        icon: '🗣️', title: 'Text-to-Speech',     desc: 'Turn text into audio (voice from Settings)',     feature: 'tts',        handler: 'coming-soon' },
    { id: 'code',       icon: '💻', title: 'Code Agent',         desc: 'Compile & convert code across languages',        feature: 'code',       handler: 'coming-soon' },
    { id: 'web',        icon: '🌐', title: 'Web / Internet',     desc: 'Ask GPT-5.4 / Mistral with web search + x402',   feature: 'web',        handler: 'coming-soon' },
  ...
  function renderQuota() {
    ...
    if (snap[0]) dateEl.textContent = 'Refreshes at midnight · ' + snap[0].date;
  ...
  function renderTools() {
    ...
    const compact = document.body.classList.contains('compact-cards');
    ...
    grid.innerHTML = TOOLS.map(t => {
      ...
      return `
        <button
          data-tool-id="${t.id}"
          ${disabled ? 'disabled' : ''}
          class="tool-card text-left rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 ${compact ? 'p-3' : 'p-4'}
                 hover:shadow-md disabled:opacity-60 disabled:cursor-not-allowed">
          <div class="flex items-center justify-between">
            <div class="${compact ? 'text-xl' : 'text-2xl'}">${t.icon}</div>
          ...
          ${compact ? '' : `<div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">${escapeHtml(t.desc)}</div>`}
        </button>`;
    }).join('');
  ...
  function openTool(id) {
    ...
    if (typeof tool.handler === 'function') {
      ...
      body.innerHTML = `
        <div class="text-sm text-slate-500 dark:text-slate-400 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-6 text-center">
          <div class="text-3xl mb-2">${tool.icon}</div>
          <div class="font-medium text-slate-700 dark:text-slate-200">${escapeHtml(tool.title)}</div>
  ...
  // Public re-render entry point (login, settings save, quota change).
  window.Hub = {
    refresh() { renderQuota(); renderTools(); },
    openTool,
    closeWorkspace
  };
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // (including the automatic midnight refresh).
