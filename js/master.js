 ...
 *
 * CHAT HISTORY (added): the request body may now include a `history` array
 * — prior turns of the same session, oldest first, as sent by
 * master-client.js's chat-history sidebar. It's sanitized via
 * sanitizeHistory() below and threaded into BOTH the routing call
 * (callOpenRouterRoute) and, for agent_id chat/web, the generation call
 * (callOpenRouterGenerate) — so a follow-up like "make it shorter" or "now
 * turn that into an image" resolves against the actual conversation instead
 * of being routed/answered blind. code.convert.* deliberately does NOT get
 * history — it converts one code block per call, prior turns aren't
 * relevant. Omitting `history` entirely still works exactly as before.
 * ========================================================================= */
...
module.exports = async function handler(req, res) {
  ...
  //    `history` is passed so follow-up turns route against real context.
  let route = null, modelUsed = null, fallbackUsed = false;
  const routeErrors = [];
  for (const m of ROUTER_MODELS) {
    try {
      const parsed = await callOpenRouterRoute(apiKey, m, message, history);
  ...
  if (route.agent_id === 'chat' || route.agent_id === 'web') {
    const exec = await runTextAgent(apiKey, route.agent_id, message, history);
    res.status(200).json({
 ...
 * ------------------------------------------------------------------------- */
async function runTextAgent(apiKey, agentId, message, history) {
  const useWebPlugin = agentId === 'web' && WEB_SEARCH_FALLBACK_ENABLED;
  const attempts = [];

  for (const m of ROUTER_MODELS) {
    try {
      const gen = await callOpenRouterGenerate(apiKey, m, message, useWebPlugin, undefined, undefined, history);
      if (gen.text) {
        return {
 ...
 * ------------------------------------------------------------------------- */
async function callOpenRouterGenerate(apiKey, modelCfg, message, useWebPlugin, systemPrompt, maxTokens, history) {
  const messages = [
 ...
 * ------------------------------------------------------------------------- */
async function callOpenRouterRoute(apiKey, modelCfg, message, history) {
...
// Chat-history guard: cap how many prior turns and how many characters get
// forwarded to OpenRouter. Keeps latency/cost bounded regardless of how long
// a session has grown client-side, and drops anything malformed rather than
// erroring the request. Most recent turns are kept (oldest ones dropped
// first) since those are the most relevant to a follow-up message.
