/* lib/analyze-attachment.js — vision/OCR/text analysis + file edit (streamable) */
  ...
  for (var i = 0; i < models.length; i++) {
    ...
    try {
      ...
      var resp = await fetch(ZHIPU_URL, {
        ...
        body: JSON.stringify({ model: model, messages: messages, max_tokens: 16000 }),
  ...
  for (var i = 0; i < visionModels.length; i++) {
    ...
    try {
      ...
      var resp = await fetch(OPENROUTER_URL, {
        ...
        body: JSON.stringify({ model: vm.model, messages: messages, max_tokens: 16000 }),
        signal: AbortSignal.timeout(120000)
      });
      if (!resp.ok) {
        var detail = await safeText(resp);
...
/**
 * @param tryGenerateAnswer(msg, history, prefs, memory, opts) — opts may include onDelta for SSE
 */
async function analyzeAttachment(message, attachment, history, prefs, tryGenerateAnswer, streamOpts) {
  streamOpts = streamOpts || {};
  var onDelta = typeof streamOpts.onDelta === 'function' ? streamOpts.onDelta : null;
  var attempts = [];
  ...
  if (attachment.kind === 'text' && attachment.text) {
    var clip = String(attachment.text).slice(0, 200000);
    var editMode = wantsFileEdit(message);
    thinkingLines.push(editMode
      ? '5. Edit mode ON — applying requested changes and returning the full modified file (streaming).'
      : '5. Analyze/summarize mode (streaming).');
    var userContent = buildTextFilePrompt(message, attachment, clip, editMode);
    var gen = await tryGenerateAnswer(userContent, history, prefs, null, {
      web: false,
      code: true,
      edit: editMode,
      onDelta: onDelta
    ...
    if (editMode && outText && looksLikeUnchangedEcho(clip, outText) && typeof tryGenerateAnswer === 'function') {
      ...
      if (onDelta) onDelta('\n\n_Retrying edit with stricter instructions…_\n\n', outText, {});
      var retryPrompt = [
      ...
      var retry = await tryGenerateAnswer(retryPrompt, history, prefs, null, {
        web: false,
        code: true,
        edit: true,
        onDelta: onDelta
    ...
    return {
      ...
      tools_used: gen && gen.tools_used ? gen.tools_used : 0,
      continuations: gen && gen.continuations ? gen.continuations : 0
    };
  }
  ...
  if (attachment.kind === 'image' && attachment.dataUrl) {
    ...
    if (z && z.text) {
      if (onDelta) onDelta(z.text, z.text, {});
    ...
    if (or && or.text) {
      if (onDelta) onDelta(or.text, or.text, {});
      return { text: or.text, model: or.model, provider: or.provider, attempts: attempts, thinking: thinkingLines.join('\n'), server_executed: true };
    }
    thinkingLines.push('5. Vision models unavailable — using text LLM + web on the question.');
    if (typeof tryGenerateAnswer === 'function') {
      try {
        ...
        var webGen = await tryGenerateAnswer(webPrompt, history, prefs, null, { web: true, onDelta: onDelta });
