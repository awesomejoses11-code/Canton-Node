      ...
      var messages = [
        {
          ...
          content:
            ...
            'and answer the user question completely. Never truncate mid-sentence. ' +
            'If it is a meme, name it and give origin when you know it.'
        },
        {
          role: 'user',
          content: [
...
async function analyzeAttachment(message, attachment, history, prefs, tryGenerateAnswer) {
  ...
  if (attachment.kind === 'text' && attachment.text) {
    ...
    var userContent =
      ...
      clip;
    var gen = await tryGenerateAnswer(userContent, history, prefs, null, { web: false, code: true });
    return {
      text: gen.text,
      model: gen.model,
  ...
  if (attachment.kind === 'image' && attachment.dataUrl) {
    var prompt = String(
      message || 'Analyze this image. Describe it, OCR any text, and note important details. Be complete.'
    );
    ...
    if (typeof tryGenerateAnswer === 'function') {
      try {
        var webPrompt =
          ...
          'Be honest if you cannot see the pixels. Answer completely.)';
