...
async function ensureSchema() {
  ...
  // Migrate older schemas (password was required; Google-only needs nullable)
  try { await sql`ALTER TABLE users ADD COLUMN IF NOT EXISTS google_sub TEXT`; } catch (_) {}
  try { await sql`ALTER TABLE users ALTER COLUMN password_hash DROP NOT NULL`; } catch (_) {}
  try { await sql`ALTER TABLE users ALTER COLUMN salt DROP NOT NULL`; } catch (_) {}
  try {
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS users_google_sub_uidx ON users (google_sub) WHERE google_sub IS NOT NULL`;
  } catch (_) {}
